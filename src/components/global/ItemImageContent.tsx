...
export default function ItemImageContent({ name, imageSrc, width = 50, rounded } : { name: string, imageSrc: string, width: number, rounded: boolean }) {
    return (
        <div className={`overflow-hidden relative flex-shrink-0 ${rounded ? "rounded-full border-[1px] border-transparent" : ""}`}>
            ...
            <Image src={imageSrc} width={width} height={width} alt={name} priority={false} className="object-cover aspect-square" />
        </div>
    );
}